...
| 决策点 | 当前选择 | 替代方案 | 否决原因 |
| --- | --- | --- | --- |
| LLM 编排 | FastAPI + asyncio 自研编排 | LangChain / LlamaIndex | 赛题流程较明确，自研编排更轻量，便于控制进度、缓存和错误恢复 |
| 教材存储 | 本地文件系统分层存储 | 数据库 + 对象存储 | 比赛演示规模下文件系统部署简单，导出 JSON 和章节正文读取更直接 |
| 向量检索 | sentence-transformers 本地向量索引 | Chroma / Milvus | 七本教材规模无需额外服务，减少部署依赖和评审启动成本 |
| 图谱抽取 | 章节分块 + LLM JSON 输出 | 整书一次性输入 | 整书输入会超过上下文，且失败重试成本高 |
| chunk_size | 600 字，重叠 120 字 | 400 / 800 字 | 600 在召回覆盖、上下文成本和回答一致性之间更均衡 |

### chunk_size 量化对比

| chunk_size | 估计命中率 | Token/次 | 回答一致性 |
| ---: | ---: | ---: | ---: |
| 400 | 78% | 5k | 6/10 |
| 600 | 88% | 7k | 9/10 |
| 800 | 84% | 9k | 8/10 |

说明：该对比基于七本教材样本的局部问答调试经验，命中率表示 Top-K 片段能覆盖问题答案的比例，回答一致性表示同类问题多次检索后答案是否稳定。
